...
export const addFoodAndBeverage = async (req,res) => {
    ...
    const existingItem = await foodAndBeverages.findOne({
      ...
      if (existingItem) {
        return res.status(400).json({ message: "Item with the same parameters already exists" });
      }

        const newItem = new foodAndBeverages(req.body);
        const savedItem = await newItem.save();
        res.status(201).json({ message: "Food and beverage item added successfully", item: savedItem });
      } catch (error) {
        res.status(500).json({ message: "Error adding food and beverage item", error: error.message });
      }
}
...
export const getAllFoodAndBeverages = async (req, res) => {
    try {
      const items = await foodAndBeverages.find().sort({ updatedAt: -1 });;
      res.status(200).json(items);
    } catch (error) {
      res.status(500).json({ message: "Error retrieving food and beverage items", error: error.message });
    }
  };
...
export const getFoodAndBeverageById = async (req, res) => {
    try {
      const item = await foodAndBeverages.findById(req.params.id);
      
      if (!item) {
        return res.status(404).json({ message: "Food and beverage item not found" });
      }
      
      res.status(200).json(item);
    } catch (error) {
      res.status(500).json({ message: "Error retrieving food and beverage item", error: error.message });
    }
  };
...
export const updateFoodAndBeverageById = async (req, res) => {
    try {
      ...
      const updatedItem = await foodAndBeverages.findByIdAndUpdate(
      ...
      res.status(200).json({ message: "Food and beverage item updated successfully", item: updatedItem });
    } catch (error) {
      res.status(500).json({ message: "Error updating food and beverage item", error: error.message });
    }
  };
...
export const deleteFoodAndBeverageById = async (req, res) => {
    try {
      const deletedItem = await foodAndBeverages.findByIdAndDelete(req.params.id);
  
      if (!deletedItem) {
        return res.status(404).json({ message: "Food and beverage item not found" });
      }
  
      res.status(200).json({ message: "Food and beverage item deleted successfully" });
    } catch (error) {
      res.status(500).json({ message: "Error deleting food and beverage item", error: error.message });
    }
  };
...
export const getLowStockFoodAndBeverages = async (req, res) => {
    try {
      ...
      res.status(500).json({ message: "Error retrieving low stock food and beverage items", error: error.message });
    }
  };
...
export const getItemsCloseToExpiry = async (req, res) => {
    try {
      ...
      if (itemsCloseToExpiry.length === 0) {
        return res.status(404).json({ message: "No items close to expiry found" });
      }
  
      res.status(200).json(itemsCloseToExpiry); // Return the items close to expiry
    } catch (error) {
      res.status(500).json({ message: "Error retrieving items close to expiry", error: error.message });
    }
  };
