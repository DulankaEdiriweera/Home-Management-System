...
const ExpenseTracker = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setError(null);
      const url = isUpdating ? `http://localhost:4000/expenses/${selectedExpenseId}` : "http://localhost:4000/expenses";
  ...
  const handleDelete = async () => {
    try {
      const response = await fetch(`http://localhost:4000/expenses/${expenseToDelete}`, { 
        method: "DELETE" 
      });
      
      if (!response.ok) throw new Error("Failed to delete expense");
      
      // Fetch fresh data after deletion - ensure we're getting the latest data
      await fetchExpenses();
        ...
        <table className="w-full border border-gray-300 text-center rounded-lg overflow-hidden">
          ...
          <tbody>
            {filteredExpenses.length === 0 ? (
              <tr>
                <td colSpan="7" className="p-4 text-center">No expenses found. Add one to get started!</td>
              </tr>
            ) : (
              <>
                {filteredExpenses.map((expense) => (
      ...
      {showModal && (
        <div className="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50 z-50">
          <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-md">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-2xl font-semibold">{isUpdating ? "Update" : "Add"} Expense</h2>
              <button 
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-3">
              ...
              <div>
                ...
                <input 
                  ...
                  required 
                  className="w-full p-2 border rounded-lg focus:ring-blue-500 focus:border-blue-500"
                  min={formData.month ? `${getCurrentYear()}-${getMonthNumber(formData.month)}-01` : ""}
                  max={formData.month ? `${getCurrentYear()}-${getMonthNumber(formData.month)}-${getLastDayOfMonth(formData.month)}` : ""}
