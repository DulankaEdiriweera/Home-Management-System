...
const AutoRecipeGenerator = () => {
    ...
    const fetchExpiringItemsAndGenerateRecipes = async () => {
        try {
            ...
            if (formattedData.length > 0) {
                ...
                const suggestions = createRecipeSuggestions(categorizedItems, formattedData);
            ...
            setError(
                err.response?.data?.message || "Error fetching expiring items"
            );
        } finally {
            setLoading(false);
    ...
    const regenerateRecipes = () => {
        ...
        const newSuggestions = createRecipeSuggestions(categorizedItems, expiringItems, true);
    ...
    const createRecipeSuggestions = (categorizedItems, allItems, isRegeneration = false) => {
        const recipes = [];
        ...
        if (isRegeneration) {
            for (let i = recipeTemplates.length - 1; i > 0; i--) {
                ...
                [recipeTemplates[i], recipeTemplates[j]] = [recipeTemplates[j], recipeTemplates[i]];
            }
        }

        recipeTemplates.forEach(template => {
            const hasRequiredIngredients = template.requiredCategories.every(
                category => categorizedItems[category] && categorizedItems[category].length > 0
            );
        ...
        const unusedItems = allItems.filter(item => !usedIngredients.has(item.itemName));
        ...
        if (unusedItems.length > 0) {
            const customRecipe = {
                ...
                instructions: "Combine the ingredients based on their compatibility to create a simple dish.",
                difficulty: "Easy",
    ...
    return (
        <div className="flex flex-col p-6 bg-gray-100 min-h-screen">
            ...
            <div className="flex items-center mb-6">
                <button
                    className="bg-blue-500 text-white p-2 rounded-full mr-4 hover:bg-blue-600 shadow-md"
            ...
            {loading ? (
                ...
                <div className="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded mb-4">
                    <span className="block sm:inline">No items are about to expire within 3 days.</span>
                </div>
            ) : (
                <>
                    ...
                    <div className="flex justify-between mb-6">
                        <button
                            className={`${regenerating ? 'bg-gray-400' : 'bg-blue-500 hover:bg-purple-700'} text-white px-4 py-2 rounded-xl font-semibold flex items-center gap-2 shadow-md transition`}
                    ...
                    {generatedRecipes.length === 0 ? (
                        <div className="bg-yellow-100 border border-yellow-400 text-yellow-700 px-4 py-3 rounded mb-4">
                            <span className="block sm:inline">
                                Unable to generate recipes with the current expiring ingredients.
                            </span>
                        </div>
                    ) : (
                        <>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                                {generatedRecipes.map((recipe, index) => (
                                    <div key={index} className="bg-white rounded-xl shadow-md overflow-hidden">
                                        ...
                                        <div className="p-6">
                                            <div className="mb-4">
                                                <h3 className="text-lg font-semibold mb-2 border-b border-gray-200 pb-2">Ingredients</h3>
                                                <ul className="space-y-2">
                                                    {recipe.ingredients.map((ingredient, idx) => (
                                                        <li key={idx} className="flex items-start">
                                                            <span className="inline-block w-2 h-2 rounded-full bg-orange-500 mt-2 mr-2"></span>
                                                            <div>
                                                                <span className="font-medium">{ingredient.quantity} * {ingredient.weightVolume}{ingredient.unit} {ingredient.name}</span>
                                                                <span className="text-sm text-red-500 ml-2">
                                                                    (expires in {ingredient.daysToExpiry} days)
                                                                </span>
                                                            </div>
                                                        </li>
                                                    ))}
                                            ...
                                            <div>
                                                <h3 className="text-lg font-semibold mb-2 border-b border-gray-200 pb-2">Instructions</h3>
                                                <ol className="space-y-2">
                                                    {recipe.instructions.split('\n').map((instruction, idx) => (
                                                        <li key={idx} className="flex items-start">
                                                            <span className="inline-flex items-center justify-center w-6 h-6 rounded-full bg-blue-100 text-blue-600 font-medium text-sm mr-2">{idx + 1}</span>
